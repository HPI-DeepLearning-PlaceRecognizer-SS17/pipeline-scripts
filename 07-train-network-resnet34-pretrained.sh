...
python $ssdRoot/train.py --train-path $ssdDataDir/train.rec --val-path $ssdDataDir/val.rec --prefix $ssdRoot/model/ssdResNet34pretr --pretrained $ssdRoot/imagenet1k-resnet-34 --epoch 0 --batch-size 8 --num-class 8 --class-names berlinerdom,brandenburgertor,fernsehturm,funkturm,none,reichstag,rotesrathaus,siegessaeule --lr 0.002 --gpus 0 --log $rootDir/training-log/$(date +%F-%T)-train_resnet_34_allcl.log --end-epoch 30 --network resnet34_ssd_300
